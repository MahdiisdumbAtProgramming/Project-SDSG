const player1=document.getElementById("player1");const player2=document.getElementById("player2");const tagStatus=document.getElementById("tag-status");const gamearea=document.getElementById("game-area");let player1Pos={x:100,y:100};let player2Pos={x:400,y:100};let isPlayer1It=!0;let tagDelayActive=!1;const baseSpeed=5;const slowSpeed=1;let currentSpeed=baseSpeed;let speedDecayInterval;const speedDecayRate=0.05;const speedDecayDelay=2000;let keys={};player1.style.left=player1Pos.x+"px";player1.style.top=player1Pos.y+"px";player2.style.left=player2Pos.x+"px";player2.style.top=player2Pos.y+"px";document.addEventListener("keydown",(event)=>{keys[event.key]=!0});document.addEventListener("keyup",(event)=>{keys[event.key]=!1});function gameLoop(){if(!tagDelayActive){movePlayers()}else{moveNonItPlayer()}
...
if(keys.ArrowDown&&player1Pos.y<gamearea.clientHeight-player1.offsetHeight){player1Pos.y+=speed}
if(keys.ArrowLeft&&player1Pos.x>0){player1Pos.x-=speed}
if(keys.ArrowRight&&player1Pos.x<gamearea.clientWidth-player1.offsetWidth){player1Pos.x+=speed}
...
if(keys.s&&player2Pos.y<gamearea.clientHeight-player2.offsetHeight){player2Pos.y+=speed}
if(keys.a&&player2Pos.x>0){player2Pos.x-=speed}
if(keys.d&&player2Pos.x<gamearea.clientWidth-player2.offsetWidth){player2Pos.x+=speed}
